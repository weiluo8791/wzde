...
var elFinder = function(elm, opts, bootCallback) {
	...
	this.baseUrl = (function() {
		...
		if (self.options.baseUrl) {
			...
			if (myTag.length) {
				myCss = $('head > link[href$="css/elfinder.full.css"],link[href$="css/elfinder.full.css"]:first').length;
				if (! myCss) {
	...
	if (this.options.cssAutoLoad) {
		(function() {
			...
			if (self.cssloaded === null) {
				...
				// load CSS
				self.loadCss([baseUrl+'css/elfinder.full.css', baseUrl+'css/theme.css']);
...
if (typeof elFinder === 'function' && elFinder.prototype.i18) {
	elFinder.prototype.i18.en = {
		...
		messages   : {
			...
			// ATWEB-8197 by WLUO modified message to reflect disabled save as button
					...
					close   : function() {
						...
						if (changed()) {
							fm.confirm({
								...
								// ATWEB-8197 by WLUO Disable Save As
/* 								buttons : [{
									label    : 'btnSaveAs',
									callback : function() {
			...
			if (!makedir && existed && map[name].mime != 'directory') {
				...
				if (mkdirAll == 0) {
					fm.confirm({
						...
						// CWT-8853 - issue #7, disable No option
/* 						reject : {
							label : 'btnNo',
							callback:function (all) {
								all && (mkdirAll = -1);
