...
//config file
define("INIFILE", dirname(__FILE__) . "\\initialCommit.ini");
...
define("ZONEPATH", $ini_array["WZDE"]["ZONEPATH"]);
//ATWEB-5010 by WLUO - Allow custom comment and prop change to reflect actual author
define("DEFAULT_MESSAGE","commit from WZDE by " . substr( $_SERVER['REMOTE_USER'], strrpos( $_SERVER['REMOTE_USER'], '\\' )+1));
//ATWEB-5011 by WLUO Allow prop change to reflect actual author
...
if( !empty( $_POST ) ) {
    ...
    if (empty($_POST['message'])) {        
        ...
        $commitMessage = $_POST['message'];
        svn_command($command,$checkoutPath,$commitMessage);
    }
}

//return 0 for error, 1 for success
//optional agrument $cMessage, default = DEFAULT_MESSAGE
//ATWEB-5010 by WLUO - Allow custom comment and prop change to reflect actual author
function svn_command ($command, $cPath, $cMessage = DEFAULT_MESSAGE) {
    $checkoutPath = ZONE_ROOT . $cPath;
    
    //check if SVN committed 
    if ($command === 'info') {
        $addition_arg = COMMAND;
        $cmd = "svn info $addition_arg \"$checkoutPath\" 2>&1";
    ...
    //check if SVN update 
        ...
        $addition_arg = COMMAND;
        $cmd = "svn update $addition_arg \"$checkoutPath\" 2>&1";
        //$err has value means not committed
        exec($cmd,$out,$err);
        logEvent($cmd,$out,$err);
    }
    //check if SVN cleanup 
    else if ($command === 'cleanup') {
        $addition_arg = COMMAND;
        $cmd = "svn cleanup $addition_arg \"$checkoutPath\" 2>&1";
        //$err has value means not committed
        exec($cmd,$out,$err);
        logEvent($cmd,$out,$err);
    }	
    //COMMIT whole site
        ...
        if ($_POST['committed']==1) {
            ...
            $commitMessageFile = 'commitMessage_' . $_COOKIE["PHPSESSID"] . ".txt";
            file_put_contents($commitMessageFile, print_r($message, true), LOCK_EX);                        
            $command = COMMAND;
            $cmd = "svn --force --depth infinity add $command \"$checkoutPath\" 2>&1";
            exec($cmd,$out,$err);
            logEvent($cmd,$out,$err);
            //$cmd = "svn commit $command -m \"$message\" $checkoutPath";
            $cmd = "svn commit $command -F $commitMessageFile \"$checkoutPath\" 2>&1";
            exec($cmd,$out,$err);
            logEvent($cmd,$out,$err);
            //ATWEB-5011 by WLUO Allow prop change to reflect actual author
    ...
    //COMMIT single folder or file
        ...
        $message = "commit from WZDE by ". substr( $_SERVER['REMOTE_USER'], strrpos( $_SERVER['REMOTE_USER'], '\\' )+1 );            
        $command = COMMAND;
        $cmd = "svn commit $command -m \"$message\" \"$checkoutPath\" 2>&1";
        exec($cmd,$out,$err);
        logEvent($cmd,$out,$err);    
    }
    //ADD file or folder
    ...
    //COMMIT all queued files or folders into 1 revision
        ...
        $cmd = "svn commit $command --targets $commitList -F $commitMessageFile";        
        exec($cmd,$out,$err);
        logEvent($cmd,$out,$err);
        //ATWEB-5011 by WLUO Allow prop change to reflect actual author
        $revision = end(preg_replace('/[^0-9]/', '', $out));
        ...
        if (file_exists($commitList)) { 
            ...
            foreach ($lines as $line_num => $line) {
                ...
                //if this is a file
                    ...
                    $temp = substr($line, strlen(ZONEPATH));                    
		...
		$err = "UNKNOWN_COMMAND";
		$out = ["The SVN Command " . $command ." encountered an error.",""];
	}
    
    //echo ($err ? 0 : 1);
	//return 1 = no error
	//return json_encode($out) = error
